    ...
    for i in range(len(VecBx)): # Checar se o formato do arquivo está adequado e se o Timestamp dos arquiveos bates e converter de mG para uT.
        try:
            if len(VecBx[i]["Satellite Timestamp (UTC)"]) == len(VecBy[i]["Satellite Timestamp (UTC)"]) == len(VecBz[i]["Satellite Timestamp (UTC)"]):
                ...
                VecBxyz += [{"Bx":0.1*VecBx[i]["Value (mG)"],"By":0.1*VecBy[i]["Value (mG)"],"Bz":0.1*VecBz[i]["Value (mG)"],"t":(floatdata - floatdata0)/20}]
            else:
...
def Magcontroler(data,window): # Essa função tem por objetivo controlar o campo magnético em cada um dos eixos.
    ...
    while Control == True:     # Iniciar o controle do sistema sempre que a variável Control estiver em True.
        try:
            ...
            if (abs(Bref[0] - Bx)>2) or (abs(Bref[1] - By)>2) or (abs(Bref[2] - Bz)>2)and(Estimardor == True ):
...
def Vset(Vx,Vy,Vz):
    ...
    ga.write(f"VSET {abs(Vy):.2f}") 
    gb.write(f"VOLT {abs(Vz):.2f}")
    gc.write(f"VSET {abs(Vx):.2f}")
    inv = Inv(Vx,Vy,Vz)
    a = arduino.write(inv)
...
def main():
    ...
    cbox1_layout = [[sg.T('CONFIGURAÇÃO', text_color='white',justification = "center")],
                    ...
                    [sg.T('MAGNETÔMETRO:\t', justification = "center", text_color='white',background_color = "royalblue4"),sg.Combo(PortasAbertas, size=(7, 1), key='-LIST2-',enable_events = True),sg.Button(image_data =recarregar,key = '-IMG2-')],
                    [sg.T('FONTE A:\t\t', justification = "center", text_color='white',background_color = "royalblue4"),sg.Button(image_data =recarregar, key = '-IMG3-')],
    ...
    cbox2_layout = [[sg.T('PROPAGAÇÃO', text_color='white',justification = "center")],
                    ...
                    [sg.Text('CAMPO BOBINA X [uT]',background_color = "royalblue4", size=(17, 1)), sg.Input(size=(10, 1),key ='-BX-'),sg.Text('Hx = 0.00 [uT]',background_color = "royalblue4", size=(15, 1),key ='-HX-')],
                    [sg.Text('CAMPO BOBINA Y [uT]',background_color = "royalblue4", size=(17, 1)), sg.Input(size=(10, 1),key ='-BY-'),sg.Text('Hy = 0.00 [uT]',background_color = "royalblue4", size=(15, 1),key ='-HY-')],
                    [sg.Text('CAMPO BOBINA Z [uT]',background_color = "royalblue4", size=(17, 1)), sg.Input(size=(10, 1),key ='-BZ-'),sg.Text('Hz = 0.00 [uT]',background_color = "royalblue4", size=(15, 1),key ='-HZ-')],
                    ...
                    [sg.Text('KD',background_color = "royalblue4", size=(2, 1)), sg.Input(default_text ="0.45",size=(10, 1),key ='-KD-'),sg.Text('KI',background_color = "royalblue4", size=(2, 1)), sg.Input(default_text ="1.00",size=(10, 1),key ='-KI-')],
                    ...
                    [sg.Text('PERÍODO DE AMOSTRAGEM',background_color = "royalblue4", size=(25, 1)), sg.Input(default_text ="1.00",size=(10, 1),key ='-T-',enable_events = True)],
    ...
    window = sg.Window('GAIOLA DE HELMOTZ v3.0', layout, font='_ 9',resizable = True, finalize=True,auto_size_text = True, auto_size_buttons = True)
    if a != "":
        print(a)
    while True:             # Event Loop
        ...
        if '-LIST2-' in event:
            if magnet != []:
                try:
                    magnet.close()
                    magnet = serial.Serial(port=values["-LIST2-"], baudrate=9600, timeout=0.1)
