...
def main():
    ...
    cbox2_layout = [[sg.T('PROPAGAÇÃO', text_color='white',justification = "center")],
                    ...
                    [sg.Text('PERÍODO DE AMOSTRAGEM',background_color = "royalblue4", size=(25, 1)), sg.Input(default_text ="0.50",size=(10, 1),key ='-T-',enable_events = True)],
    ...
    while True:             # Event Loop
        ...
        if '-LIST2-' in event:
            if magnet != []:
                try:
                    magnet.close()
                    magnet = serial.Serial(port=values["-LIST2-"], baudrate=9600, timeout=0.1,write_timeout = 0.1)#xonxoff=True
